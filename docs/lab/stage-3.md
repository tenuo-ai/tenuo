...
<header class="lab-hero"><div class="lab-kicker">Stage 3 of 9 · <span class="lab-mode scoped">scoped</span> · about 15 min</div><h1>Rules that fit the job</h1><p class="lab-goal"><strong>Goal.</strong> Write the permissions yourself, narrow enough that every rogue action is blocked and the trip still books.</p></header>

<p class="lab-intro">Instead of &quot;Check-in Agent can read reservations,&quot; you say <em>which</em> reservation. Instead of &quot;Flight Agent can book flights,&quot; you say <em>which</em> destination and <em>how much</em>.</p>

<figure class="lab-figure"><svg class="lab-diagram" viewBox="0 0 760 296" role="img" aria-label="Each rule names the job: the destination, the reservation, the ceiling." xmlns="http://www.w3.org/2000/svg"><path d="M100 78 L100 222" fill="none" stroke="#5a5a5a" stroke-width="1.5"/><path d="M100 138 L215 138" fill="none" stroke="#5a5a5a" stroke-width="1.5"/><polygon points="215,138 207,133 207,143" fill="#5a5a5a"/><path d="M100 222 L215 222" fill="none" stroke="#5a5a5a" stroke-width="1.5"/><polygon points="215,222 207,217 207,227" fill="#5a5a5a"/><path d="M170 54 L215 54" fill="none" stroke="#5a5a5a" stroke-width="1.5"/><polygon points="215,54 207,49 207,59" fill="#5a5a5a"/><path d="M355 54 L400 54" fill="none" stroke="#5a5a5a" stroke-width="1.5"/><polygon points="400,54 392,49 392,59" fill="#5a5a5a"/><path d="M540 54 L585 54" fill="none" stroke="#5a5a5a" stroke-width="1.5"/><polygon points="585,54 577,49 577,59" fill="#5a5a5a"/><rect x="30" y="30" width="140" height="48" rx="8" fill="var(--surface-2)" stroke="var(--border)" stroke-width="1"/><text x="42" y="51" font-size="13" font-weight="600" fill="var(--text)">Travel Agent</text><text x="42" y="68" font-size="11" fill="var(--text-muted)">name only, calendar</text><rect x="215" y="30" width="140" height="48" rx="8" fill="var(--surface-2)" stroke="var(--border)" stroke-width="1"/><text x="227" y="51" font-size="13" font-weight="600" fill="var(--text)">Flight Agent</text><text x="227" y="68" font-size="11" fill="var(--text-muted)">CUN, ≤ $300</text><rect x="215" y="114" width="140" height="48" rx="8" fill="var(--surface-2)" stroke="var(--border)" stroke-width="1"/><text x="227" y="135" font-size="13" font-weight="600" fill="var(--text)">Hotel Agent</text><text x="227" y="152" font-size="11" fill="var(--text-muted)">Cancún, ≤ $200/night</text><rect x="215" y="198" width="140" height="48" rx="8" fill="var(--surface-2)" stroke="var(--border)" stroke-width="1"/><text x="227" y="219" font-size="13" font-weight="600" fill="var(--text)">Activity Agent</text><text x="227" y="236" font-size="11" fill="var(--text-muted)">Cancún, ≤ $200</text><rect x="400" y="30" width="140" height="48" rx="8" fill="var(--surface-2)" stroke="#ff5c5c" stroke-width="2"/><text x="412" y="51" font-size="13" font-weight="600" fill="var(--text)">Check-in Agent</text><text x="412" y="68" font-size="11" fill="var(--text-muted)">UA214 only</text><rect x="490" y="21" width="44" height="16" rx="8" fill="#ff5c5c"/><text x="512" y="33" font-size="10" font-weight="700" text-anchor="middle" fill="#0a0a0a">rogue</text><rect x="585" y="30" width="140" height="48" rx="8" fill="var(--surface-2)" stroke="var(--border)" stroke-width="1"/><text x="597" y="51" font-size="13" font-weight="600" fill="var(--text)">Boarding Agent</text><text x="597" y="68" font-size="11" fill="var(--text-muted)">UA214 only</text><rect x="585" y="208" width="110.39999999999999" height="26" rx="13" fill="var(--surface)" stroke="#ffb000" stroke-width="1.5"/><text x="640.2" y="225" font-size="12" text-anchor="middle" fill="var(--text)">Wallet $1,200</text><text x="380" y="288" font-size="12" text-anchor="middle" fill="var(--text-muted)">Each rule names the job: the destination, the reservation, the ceiling.</text></svg></figure>
...
Stage 3 of 9: Rules that fit the job   mode=scoped  scenario=spring-break
...
<details class="lab-reveal answer"><summary>Show a reference solution</summary><div><p class="lab-muted">Try yours first. The score does not care whether it matches this one.</p><figure class="lab-code"><figcaption>One policy that scores 100 <span>answers/03-scoped/policy.ts</span></figcaption>
{% highlight ts %}
export const config: PolicyConfig = {
  policy: {
    "travel-agent": {
...
<aside class="lab-callout stuck"><div class="lab-callout-title">If it does not work</div><ul><li><code>npm run audit</code> prints what every agent can currently do.</li><li>Blocking everything scores zero: the trip has to work. Read <strong>THE TRIP</strong> in the output first.</li><li>Every denial names the rule that fired. Read the whole line.</li></ul></aside>
...
<nav class="lab-nav"><a class="prev" href="/lab/stage-2">← Stage 2</a><a class="next" href="/lab/stage-4">Stage 4: A second traveler shows up →</a></nav>
