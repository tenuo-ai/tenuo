...
export function missionCeiling(trip: Trip): Record<AgentId, RoleCeiling> {
  return {
    ...
    "hotel-agent": {
      tools: [TOOLS.search_hotels, TOOLS.book_hotel, TOOLS.get_hotel_booking, TOOLS.wallet_charge, TOOLS.traveler_read],
      city: [trip.city],
      hotelRate: trip.hotelRateBudget,
    ...
    "checkin-agent": {
      tools: [TOOLS.get_reservation, TOOLS.check_in, TOOLS.get_checkin_status, TOOLS.issue_boarding_pass],
      reservations: "one-of-task",
    },
    "boarding-agent": {
      tools: [TOOLS.issue_boarding_pass, TOOLS.get_checkin_status],
      reservations: "one-of-task",
    },
  };
}
