...
class TenuoProtectedPipeline(BasePipelineElement):
    ...
    def __init__(
        ...
        # Merge with baseline policy
        from .task_policies import get_baseline_policy

        baseline = get_baseline_policy()
        merged_constraints = {**self.base_constraints, **baseline}
        self.warrants = self._create_warrants(merged_constraints)
        logger.info(f"Loaded {len(self.warrants)} tool warrants")
    ...
    def query(self, query, runtime, env=None, messages=None, extra_args=None):
        ...
        # Use pre-created role-based warrants
...
class TenuoAgentDojoHarness:
    ...
    benchmark framework to measure constraint enforcement accuracy.
    """
