...
class TenuoToolWrapper:
    ...
    def __call__(self, **kwargs) -> Any:
        ...
        """
        print(f"[DEBUG] TenuoToolWrapper.__call__ for {self.tool_name}, metrics_id={id(self.metrics)}")
        try:
            ...
            try:
                ...
                # Debug: which arg caused the issue?
                print(f"[DEBUG] PoP creation failed: {ve}")
                print(f"[DEBUG] Args types: {[(k, type(v).__name__, type(v[0]).__name__ if isinstance(v, list) and v else None) for k, v in filtered_args.items()]}")
                raise
            
            print(f"[DEBUG] PoP signature created for {self.tool_name}")
            ...
            print(f"[DEBUG] Authorization result for {self.tool_name}: {authorized}")
            ...
            if not authorized:
                ...
                print(f"[DEBUG] Recorded DENIED for {self.tool_name}, total denied: {self.metrics.denied}")
            ...
            print(f"[DEBUG] Recorded ALLOWED for {self.tool_name}, total allowed: {self.metrics.allowed}")
            return self.tool(**kwargs)
        except UnauthorizedToolCallError:
            raise  # Re-raise authorization errors as-is
        except Exception as e:
            print(f"[DEBUG] Exception in TenuoToolWrapper for {self.tool_name}: {type(e).__name__}: {e}")
            raise
