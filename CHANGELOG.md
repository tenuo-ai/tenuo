...
- **`tenuo._pop_canonicalize.strip_none_values(args)`** helper — a small
  ...
  values and `None` list elements (recursively, at any list-nesting depth)
  so optional tool parameters with `Optional[...] = None` defaults don't
  crash the canonicalizer.
- **Asymmetric-by-default MCP handshake fixtures** —
  `tenuo-python/tests/adapters/_mcp_test_fixtures.py` exposes
  `make_client_side` / `make_server_side` / `perform_handshake`. Each side
  owns its own `CompiledMcpConfig` (or none), so client/server drift is the
  fixture's default rather than a special case a test has to remember to
  construct.
- **FFI-boundary fuzzer** —
  `tenuo-python/tests/property/test_pop_ffi_fuzzer.py` drives Hypothesis
  over arbitrary JSON-shaped argument dicts (with `None`, nested lists,
  Unicode, booleans, large ints) to pin three invariants: `strip_none_values`
  produces a recursively `None`-free dict, the `sign → verify` round-trip
  succeeds for any stripped input, and canonicalization drift across the
  FFI surfaces as a denial or a raised exception (never silent authorization).
- **Cross-language PoP canonicalization fixture** —
  `tenuo-core/tests/fixtures/pop_canonical.json` is a deterministic
  fixture pinning the exact Ed25519 signature bytes produced by
  `Warrant::sign_with_timestamp` over a curated set of args, keys, and
  warrants. Regenerated by `tenuo-python/tests/unit/_regen_pop_canonical.py`
  and asserted from both sides by
  `tenuo-core/tests/pop_canonical_fixture.rs` and
