...
#!/usr/bin/env python3
"""
AutoGen AgentChat demo: GuardBuilder (Tier 2 warrant + PoP) using the SAME
three-agent flow as the protected/attenuation demos:

    User -> Orchestrator -> Paper Search (fetch_url) -> Summarizer (read_file)

Protection:
- Single warrant with two capabilities:
    - fetch_url constrained by UrlSafe allowlist (arxiv.org, scholar.google.com)
    - read_file constrained by Subpath("/tmp/papers")
- GuardBuilder.with_warrant(...) enforces PoP on each tool call

Run:
    python autogen_demo_guardbuilder_tier2.py
    python autogen_demo_guardbuilder_tier2.py run
    python autogen_demo_guardbuilder_tier2.py attack
    python autogen_demo_guardbuilder_tier2.py compare
...
async def run_demo(
    ...
    guard = GuardBuilder().with_warrant(warrant, issuer_key).build()
...
async def run_comparison(
    ...
    print("AUTOGEN GUARD BUILDER (TIER 2, WARRANT + PoP)")
    print(
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="AutoGen AgentChat demo - GuardBuilder Tier 2 (warrant + PoP)"
    )
    parser.add_argument(
        "command",
