#!/usr/bin/env python3
"""
AutoGen AgentChat demo: research pipeline (UNPROTECTED).
...
async def run_demo(
    ...
    if inject_attack:
        header("DEMO: AutoGen Research Pipeline (UNPROTECTED, WITH ATTACK)")
        print(
            "  This run simulates prompt injection without any protections.\n"
        )
    else:
        header("DEMO: AutoGen Research Pipeline (Normal Flow)")
        print("  This run shows normal operation without protections.\n")

    # ---------------------------------------------------------------------
    # Setup: Tools (no authorization/constraints)
    # ---------------------------------------------------------------------
    ...
    # Setup: Agents
    # ---------------------------------------------------------------------
    step(1, "Creating AutoGen agents")
    load_dotenv()
    ...
    # ---------------------------------------------------------------------
    step(2, "Orchestrator delegates task to the best agent")
    if inject_attack:
    ...
    # ---------------------------------------------------------------------
    step(3, "Orchestrator delegates task to the best agent")
    if inject_attack:
    ...
    if inject_attack:
        warn(
            "This demo is unprotected: the 'attack' actions were not blocked "
            "and could execute if real tools are enabled."
        )
...
async def run_comparison(
    ...
    header("AUTOGEN A2A-STYLE DEMO (UNPROTECTED)")
    print("  Architecture:")
