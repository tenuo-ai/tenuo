...
class TestNexusExperimentalWarning:
    """``start_nexus_operation`` emits a one-time UserWarning on first use."""
    ...
    def _fresh_outbound():
        ...
        class _FakeNext:
            def start_nexus_operation(self, input):  # noqa: A002 - matches SDK arg name
                return MagicMock()

        return _TenuoWorkflowOutboundInterceptor(_FakeNext())

    @staticmethod
    def _stub_headers(run_id: str):
        ...
        with _store_lock:
            _workflow_headers_store[run_id] = {TENUO_WARRANT_HEADER: b"\x00\x01"}

    @staticmethod
    def _clear_headers(run_id: str):
    ...
    @staticmethod
    def _fake_nexus_input():
        inp = MagicMock()
        inp.headers = None
        return inp

    def test_first_use_emits_user_warning(self):
        import warnings as _warnings

        run_id = "run-nexus-warn-1"
        self._stub_headers(run_id)
        outbound = self._fresh_outbound()

        fake_info = MagicMock()
        fake_info.workflow_id = "wf-x"
        ...
        try:
            with _warnings.catch_warnings(record=True) as caught:
                ...
                with patch("temporalio.workflow.info", return_value=fake_info):
                    outbound.start_nexus_operation(self._fake_nexus_input())

            user_warns = [w for w in caught if issubclass(w.category, UserWarning)]
            assert len(user_warns) == 1
            assert "experimental" in str(user_warns[0].message)
            assert "base64" in str(user_warns[0].message)
        finally:
            self._clear_headers(run_id)

    def test_warning_emitted_only_once_per_interceptor(self):
        import warnings as _warnings

        run_id = "run-nexus-warn-2"
        self._stub_headers(run_id)
        outbound = self._fresh_outbound()
        ...
        try:
            ...
            user_warns = [w for w in caught if issubclass(w.category, UserWarning)]
            assert len(user_warns) == 1, (
                f"expected exactly one UserWarning across multiple dispatches, "
                f"got {len(user_warns)}"
            )
        finally:
            self._clear_headers(run_id)
