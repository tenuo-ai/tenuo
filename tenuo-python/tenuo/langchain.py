...
def guard_tools(
    ...
    *,
    ...
    Example:
        ...
        kp = SigningKey.generate()
        protected = guard_tools([search, calculator], issuer_keypair=kp)
...
def guard_agent(
    ...
    *,
    ...
    1. Configures Tenuo (if keypair provided)
    ...
    Args:
        ...
        issuer_keypair: SigningKey for issuing warrants (optional, auto dev_mode)
    ...
    if not LANGCHAIN_AVAILABLE:
        raise ImportError(
            "LangChain is required for protect_agent(). "
            "Install with: pip install langchain-core"
        )
    
    # Configure if keypair provided
    if issuer_keypair is not None:
        from .config import configure, is_configured
        if not is_configured():
            configure(
    ...
    if not tools:
        logger.warning(
            "protect_agent: No tools found in agent. "
