...
class UrlSafe:
    ...
    Usage:
        from tenuo.openai import UrlSafe, guard

        # Secure defaults — blocks known SSRF vectors
        client = guard(
            openai.OpenAI(),
            constraints={"fetch": {"url": UrlSafe()}}
        )
        ...
        client = guard(
            ...
            constraints={
                "fetch": {
                    "url": UrlSafe(allow_domains=["api.github.com", "*.googleapis.com"])
                }
            }
        )
    ...
    Defense in Depth:
        ...
        Example:
            # Validation layer (Tenuo)
            client = guard(openai.OpenAI(), constraints={
                "fetch": {"url": UrlSafe(allow_domains=["*.api.com"])}
            })

            # Execution layer (tool implementation)
            from url_jail import fetch_safe

            def fetch(url: str) -> str:
                # DNS pinning, redirect validation, timeout enforcement
                response = fetch_safe(url, allow_domains=["*.api.com"])
...
# Type alias for constraint types (including Subpath, UrlSafe)
Constraint = Union[
