...
impl DataPlane {
    ...
    fn verify_chain_with_options_inner(
        ...
        // linkage, or the chain is malformed (AAT draft §7 steps 3c/3d).
        //
        ...
        // anchor's direct signature. That is a documented difference from the
        // AAT JWS profile, which requires a full chain from a depth-0 root.
        if chain.len() > 1 {
            if root.depth() != 0 {
                return Err(Error::ChainVerificationFailed(format!(
    ...
    fn verify_chain_link(&self, parent: &Warrant, child: &Warrant) -> Result<()> {
        ...
        // existed (AAT draft §4.4, §7 step 4j).
...
impl Authorizer {
    ...
    ///
    /// This is the enforcement-point analogue of the AAT draft's
    /// MAX_TOKEN_LIFETIME (§4.4, §7 step 3h). Default: no ceiling.
...
mod tests {
    ...
    fn test_gated_tool_passes_with_valid_approval() {
        ...
        let now = Utc::now();
        let approval_payload = ApprovalPayload::new(
            request_hash,
            rand::random(),
            "test-approver".into(),
            now,
    ...
    fn test_approval_gate_without_required_approvers_is_config_error() {
        ...
        assert!(
            ...
            "should report missing approvers config, got: {}",
            err
        );
    }
    ...
    fn test_approval_gate_bypass_with_garbage_approvals_blocked() {
        ...
        let garbage_approval = crate::approval::SignedApproval::create(payload, &random_key);
        ...
        assert!(
            err.to_string().contains("no required_approvers configured"),
            "should catch missing approvers before processing garbage approvals, got: {}",
            err
        );
    }
    ...
    fn test_approval_gate_propagation_through_delegation() {
        ...
        // Child only gets email.read and email.delete (not exec)
        let child = parent
            .attenuate()
            .capability("email.read", ConstraintSet::new())
            .capability("email.delete", ConstraintSet::new())
