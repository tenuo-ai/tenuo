//! Regression tests for verifier checks the AAT draft requires (§7) that were
//! found missing by running the AAT JWS vector suite against tenuo-core:
//!
//! - root shape for multi-warrant chains: depth 0, no parent_hash (steps 3c, 3d);
//!   a lone warrant stays exempt (Tenuo leaf format)
//! - issuance ordering: child.issued_at >= parent.issued_at (step 4j, I3)
//! - lifetime ceiling on the public Authorizer (step 3h)
//! - empty `All` / `Any` composites are invalid (-02 §4.5)
#![allow(deprecated)]
...
/// Documented profile difference from the AAT JWS profile: Tenuo's wire
/// format permits an intermediate warrant to be presented alone
...
fn empty_composites_are_invalid() {
    ...
    // attenuation: empty on either side is rejected.
    assert!(Any::new(vec![pdf.clone()])
        .validate_attenuation(&Any::new(vec![]))
        .is_err());
    assert!(Any::new(vec![])
        .validate_attenuation(&Any::new(vec![pdf.clone()]))
        .is_err());
